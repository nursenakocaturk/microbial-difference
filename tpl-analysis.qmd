...
Elimizde `r length(deleted)` tane, düşük çokluğa sahip takson bulunmaktadır.
...
```{r fig.cap = "Read depth values of the tested fastq files (million reads)", eval = F, include = F}
...
## Dentin ve Cementum arasında anlamlıd değişen mikroplar
...
```{r}
significant <- dds_results[dds_results$padj < 0.001, ]
```
