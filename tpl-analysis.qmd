...
```{r}
ggplot(data = shannon, aes(x = Shannon)) + geom_density(aes(fill = Tissue), alpha = 0.7)
```
...
```{r}
test.shannon <- t.test(x = Dentin_Shannon, y = Cementum_Shannon)
test.shannon
```
...
T-test için iki hipotez vardır:
...
Test sonucunda elde edilen p-değeri, sıfır hipotezinin oluşma olasılığını ifade etmektedir.

P-değeri 0.3'tür ve düşük bir değerdir. Yani bu iki dağılımın ortalamalarının aynı olması düşük bir olasılık olabilir. Bu olasılığın anlamlı olup olmadığını öğrenmek için ise belirli alpha değerleri kullanılır:
...
0.0001 alpha değeri altında elde edilen sonuç anlamlı mı?

```{r}
test.shannon$p.value < 0.0001
```
...
Bunun anlamı: 0.0001 hata olasılığı durumunda, bu iki dağılımın ortalamaları anlamlı bir şekilde farklı. Yani sıfır hipotezini reddedip, alternatif hipotezi kabul ediyoruz.
...
```{r}
ggplot(data = simpson, aes(x = Simpson)) + geom_density(aes(fill = Tissue), alpha = 0.7)
```
...
```{r}
test.simpson <- t.test(x = Dentin_Simpson, y = Cementum_Simpson)
test.simpson
```

#### Test Invsimpson

Dentin ve cementum dokulaının, invsimpson değerleri anlamlı olarak birbirinden farklı mı?

```{r}
ggplot(data = invsimpson, aes(x = Invsimpson)) + geom_density(aes(fill = Tissue), alpha = 0.7)
```
...
```{r}
test.invsimpson <- t.test(x = Dentin_Invsimpson, y = Cementum_Invsimpson)
test.invsimpson
```
