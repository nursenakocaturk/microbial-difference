...
+ Rastgele seçilmiş bir bireyin kimlik tahmini

+ Dizilerdeki entropiyi (değişkenin olası sonuçlarına özgü bilgi ve belirsizliğin ortalama düzeyidir) ölçmektedir. 

+ İncelenen dizide ne kadar fazla tür gözlemlenirse ve bolluklar ne kadar eşit olursa, diğer okumaya bakıldığında bir sonraki adımda hangi türün gözlemleneceğini tahmin etmek o kadar zordur.

+ Shannon entropisi bu tahminle ilişkili belirsizliği ölçer.
...
Verinin sütun ve satırlarının yerini değiştirmek için transpoze etmemiz gerekir:

```{r}
veri2 <- t(veri)
```

Örnekleri Sample_ID olarak isimlendirelim.

```{r}

write.table(
  x = data.frame(Sample_ID = seq(1,nrow(veri2)), 
                 Species = rownames(veri2)), 
  file = "data/otu_metadata.txt", quote = F, row.names = F, sep = "\t")

rownames(veri2) <- seq(1,nrow(veri2))
veri2 <- as_tibble(cbind(Sample_ID = seq(1,nrow(veri2)), veri2))
```


Verideki shannon, simpson ve invsimpson değerlerini 

```{r}
shannon <- data.frame(Sample_ID = veri2$Sample_ID,
           Shannon =  diversity(x = veri2[,-1],
                                index = 'shannon'
                                ))
```
...
```{r}
simpson <- data.frame(Sample_ID = veri2$Sample_ID,
           Simpson =  diversity(x = veri2[,-1],
                                index = 'simpson'
                                ))
```
...
```{r}
invsimpson <- data.frame(Sample_ID = veri2$Sample_ID,
           invsimpson =  diversity(x = veri2[,-1],
                                index = 'invsimpson'
                                ))
```


Shannon, simspon ve invsimpson değerlerini gösteren verileri yükleyelim:
...
Dokuların çeşitlilik farkını gösteren garfikleri oluşturalım:
...
```{r}
ggplot(data = shannon, aes(x = Sample_ID, y = Shannon, color = Tissue)) + geom_point() 
```
