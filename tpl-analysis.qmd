...
```{r}
ggplot(data = metadata, aes(x = Sample, y = Tissue)) + geom_boxplot()
```
...
Bu adımda ise, örneklerin sahip oldukları mikrobiyal uzaklıkları hesaplayarak, elde edilen uzaklık matrisini görselleştirelim.
